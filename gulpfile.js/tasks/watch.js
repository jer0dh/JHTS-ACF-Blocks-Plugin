...
gulp.task('watch-styles-tasks', function(cb) {
   return gulpSequence('bump-and-push', 'styles-sass-min', 'styles-sass-max', 'styles-deploy', 'size', cb);
});

gulp.task('watch-javascript-tasks', function(cb) {
   return gulpSequence('bump-and-push', 'js-vendor-scripts-assets', 'js-other-scripts-assets','js-other-scripts-minify', 'js-concat-scripts', 'js-deploy', 'size', cb);
});

gulp.task('watch-javascript-app-tasks', function(cb) {
   return gulpSequence('bump-and-push', 'js-app', 'js-deploy', 'size', cb);
});
...
gulp.task('watch', function() {
    ...
    gulp.watch([config.srcFolder + '/**/*.js'], ['watch-javascript-tasks']);
